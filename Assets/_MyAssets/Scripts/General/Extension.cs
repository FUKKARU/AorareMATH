...
namespace General.Extension
{
    ...
    internal static class Extension
    {
        ...
        internal static async UniTask SecAwaitThenDo(this float sec, Action act, Ct ct)
        {
            await sec.SecAwait(ct);
            act?.Invoke();
        }
        internal static async UniTask SecAwaitThenAwait(this float sec, Func<Ct, UniTask> task, Ct ct)
        {
            await sec.SecAwait(ct);
            if (task != null)
                await task(ct);
